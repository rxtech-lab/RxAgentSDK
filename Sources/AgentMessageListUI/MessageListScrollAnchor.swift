...
public nonisolated struct MessageListScrollAnchor: Equatable {
    public var threshold: CGFloat
    public private(set) var isNearBottom: Bool
    public private(set) var lastContentHeight: CGFloat
    ...
    public init(threshold: CGFloat = 120, isNearBottom: Bool = true) {
    ...
    @discardableResult
    public mutating func apply(contentHeight: CGFloat, visibleMaxY: CGFloat, isUserDriven: Bool) -> Decision {
    ...
    public mutating func resetToBottom() {
        isNearBottom = true
    }
}
