...
#Preview("MessageList / Pagination") {
    MessageListPreviewHarness(enablesPagination: true)
}

/// A viewport small enough that the reserved tail spacer is obvious.
///
/// Send a user message, then stream: the prompt should pin to the top and stay
/// there while the response fills the space beneath it. This mirrors the
/// geometry `MessageListPinnedTurnSwiftUITests` asserts headlessly.
#Preview("MessageList / Pinned turn (small viewport)") {
    MessageListPreviewHarness()
